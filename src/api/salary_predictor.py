...
DB_CONFIG = {
    "host":     os.getenv("POSTGRES_HOST",     "postgres_db"),   # nom du service Docker, pas localhost
    "port":     int(os.getenv("POSTGRES_PORT", "5432")),      # port interne Docker (pas 5433)
...
CACHE_DIR  = Path(os.getenv("CACHE_DIR", "/app/cache"))   # même dossier que offres_vectors.npy
...
def load_offres_from_db() -> pd.DataFrame:
    """
    Charge les offres depuis raw_offres (22 colonnes, pas de jointure nécessaire).
    Schéma réel : offre_id, titre, type_contrat, experience, description,
                  ville, region, code_postal, latitude, longitude, pays,
                  entreprise, secteur, taille_entreprise, salaire_libelle,
                  salaire_min, salaire_max, rome_code, rome_libelle,
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # — Texte combiné titre + rome_libelle + début de description ————————————
    df["texte"] = (
        df["titre"].fillna("") + " " +
        df["rome_libelle"].fillna("") + " " +
...
def _extract_salary_target(df: pd.DataFrame) -> pd.Series:
    """
    Construit la cible numérique (salaire mensuel EUR) depuis salaire_min / salaire_max / salaire_libelle.
    Règles :
      - Si salaire_min ET salaire_max connus → moyenne
      - Si un seul → utiliser celui-là
      - Si valeur > 15 000 → supposé annuel → diviser par 12
      - Sinon → parser salaire_libelle avec regex
    """
    targets = []
...
def _parse_salary_row(row) -> float | None:
    ...
    texte = str(row.get("salaire_libelle", "") or "")
...
# ─── Cache mémoire du modèle ────────────────────────────────────────────────

_load_model_cache = None


def load_model_fast() -> Pipeline:
    """Charge le modèle depuis le cache mémoire (évite les I/O disque répétées)."""
    global _load_model_cache
    if _load_model_cache is None:
        _load_model_cache = load_model()
