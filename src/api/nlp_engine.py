...
class NLPEngine:
    ...
    def vectorize_offres(self):
        """Charge et vectorise toutes les offres depuis PostgreSQL."""
        ...
        for row in rows:
            offre = {
                ...
                "salaire_min": row[9],
                "salaire_max": row[10],
                "rome_libelle":row[11] or "",
                "source":      row[12] or "",
                "url_offre":   row[13] or "",
            }
            self.offres_data.append(offre)
            
            # Texte à vectoriser = titre + description (tronqué)
            texte = f"{offre['titre']} {offre['description'][:500]}"
        ...
        log.info(f"Vectorisation terminée. Shape: {self.offres_vectors.shape}")
    ...
    def matcher_offres(self, texte_cv: str, top_k: int = 10) -> list:
        ...
        for i, offre_vec in enumerate(self.offres_vectors):
            sim = 1 - cosine(cv_vector, offre_vec)
